...
def write_files(out_file, pi_values, games):
...
def write_nodes(out_file, name_to_id):
    file_name = os.path.join(repo_root, 'datasets', 'processed_data', f'{out_file}_nodes.txt')
    with open(file_name, 'w') as file:
...
def combine_soi(sub_files, base_path):
    for dataset_id, files in sub_files.items():
        ...
        write_files(dataset_id, dataset_pi_values, dataset_games)

def convert_raw_files(file_path, read_function, title):
    dataset_games = {}
    dataset_pi_values = {}
