...
from src.archive.weighted_graph_helpers import create_hypergraph_from_data_weight, binarize_data_weighted, binarize_data_weighted_leadership
...
def synch_solve_equations_weighted(hyper_graph, pi_values, method, max_iter=MAX_ITER, sens=EPS):
    ...
    while iteration < max_iter and rms > sens:
        ...
        for s in range(len(scores)):
            if s in hyper_graph:
                games_with_player = hyper_graph[s]
...
def compute_predicted_ratings_BT_weighted(training_set, pi_values):
    bin_data = binarize_data_weighted(training_set)
    bin_bond_matrix = create_hypergraph_from_data_weight(bin_data)

    predicted_std_scores, iter = synch_solve_equations_weighted(bin_bond_matrix, MAX_ITER, pi_values, iterate_equation_newman_weighted, sens=EPS)
    return predicted_std_scores

def compute_predicted_ratings_BT_leadership_weighted(training_set, pi_values): 
    bin_data = binarize_data_weighted_leadership(training_set)
    bin_bond_matrix = create_hypergraph_from_data_weight(bin_data)

    predicted_std_scores, iter = synch_solve_equations_weighted(bin_bond_matrix, MAX_ITER, pi_values, iterate_equation_newman_leadership_weighted, sens=EPS)
    return predicted_std_scores

def compute_predicted_ratings_HO_BT_weighted(training_set, pi_values): 
    bond_matrix = create_hypergraph_from_data_weight(training_set)
    predicted_ho_scores, iter = synch_solve_equations_weighted(bond_matrix, MAX_ITER, pi_values, iterate_equation_newman_weighted, sens=EPS)
    return predicted_ho_scores


def compute_predicted_ratings_HOL_BT_weighted(training_set, pi_values):
    bond_matrix = create_hypergraph_from_data_weight(training_set)
    predicted_hol_scores, iter = synch_solve_equations_weighted(bond_matrix, MAX_ITER, pi_values, iterate_equation_newman_leadership_weighted, sens=EPS)
